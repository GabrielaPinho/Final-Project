...
def get_uniqueGenes (alltables, name_output_table, name_column):
    ...
    Initial_size = []
    if alltables == str(alltables): # if only one table was used as input
    ...
    if alltables != str(alltables): # if several tables are being compared and analyzed
        for table in alltables:
            table = pd.read_csv(table, header = 0)
            Initial_size.append(len(table[name_column]))
            SingleGenes = table.drop_duplicates([name_column]) # the output is all your table without duplicates
            NoNA = SingleGenes.dropna() # exclude NAs
            Number_genes_per_table.append(len(NoNA))
            Final_list = Final_list.append(NoNA)
        Final_list_noduplicates = Final_list.drop_duplicates([name_column]).dropna() # to exclude genes repeated among lists    
